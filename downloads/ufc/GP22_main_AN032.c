...
if((Dummy_var==0) | (Dummy_var==10))
{
    ...
    //Calculate Correction factor (ANZ_PER_CALRES)
    CLKHS_freq_corr_fact = 61.035/
      gp22_read_n_bytes(Bus_Type, 4, 0xB0, 0x00, 16) * CLKHS_freq;
   
    printf("\n Correction factor for clock = %1.3f\n", CLKHS_freq_corr_fact); 
       
    CLKHS_freq_cal *= CLKHS_freq_corr_fact; // Calibrated Clock frequency
}
...
if((Dummy_var==0) | (Dummy_var==5) | (Dummy_var==10))
{
    ...
    // RES_0 to RES_3 (Result_UP), discharge time in 탎
    ...
    // RES_0 to RES_3 (Result_DOWN), discharge time in 탎
