...
$sqlcontact = "";
$sqldl = "";
if($contact != 0){$sqlcontact = " AND info_type = ".$contact;}
if($contact == 3 && $dl != '0'){$sqldl = " AND user_msg LIKE '%".$dl."_%'";}
$sql = "SELECT * FROM user_form_info WHERE UNIX_TIMESTAMP( submit_date ) >= ".strtotime($startDate).$sqlcontact.$sqldl." ORDER BY submit_date";
...
if(! $retval )
{
  die('Could not get data: ' . mysql_error());
	...
	echo "<th><strong>Company</strong></th>";
	echo "<th><strong>Phone</strong></th>";
	echo "<th><strong>Email</strong></th>";
	echo "<th><strong>Subject</strong></th>";
	echo "<th><strong>Msg</strong></th>";
	echo "<th><strong>Date</strong></th>";
	echo "<th><strong>Region</strong></th>";
	echo "<th><strong>Type</strong></th></tr>\n";
...
echo "<br /><br /><b>CSV Link:  <a href=\"/db/" . $filename . "\">" . $filename . "</a></b><br /><br />";

}

?>

<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" id="create">

Days Back: <input type="text" name="days" value="7" /><br /><br />
...
IF Download, specific type?<br />
<select name="dl">
  <option value="0">All</option>
  <option value="tdc">TDC</option>
  <option value="pst">Picostrain</option>
  <option value="pca">Picocap</option>
  <option value="ufc">Ultrasonic</option>
  <option value="ptu">Picoturn</option>
  <option value="sur">Surface Concept</option>
</select>

<br /><br />

<input type="submit" value="generate" name="submit" id="submitButton" title="Generate File" />
